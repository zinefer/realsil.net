...
function ShowCaseLoad($workerkey)
{
  if ($workerkey)
  {
    ...
    $caseloadresult = mysql_query("SELECT *, SUBSTRING_INDEX(SUBSTRING_INDEX(`Client Name`, ' ', 2), ' ', -1) as `Last Name` FROM `caseloads` where `Worker` = '" . mysql_real_escape_string($workerkey) . "' ORDER BY `Last Name`;");
    if ($caseloadresult)
    {
      if (mysql_num_rows($caseloadresult)!=0)
      {
        ...
        {
          echo '<a href="caseload.php?remove='.$row['Client'].'">hide</a> <a href="?client='.$row['Client'].'">'.$row['Client Name'].'</a><br/>';
        }
      }else{
...
function RemoveClientFromCL($clientkey, $workerkey)
{
  $clcheck = CaseLoadCheck($clientkey, $workerkey);
  if ($clcheck == true)
  {
    $deleteresult = mysql_query("DELETE FROM `caseloads` WHERE `Worker` = '" . mysql_real_escape_string($workerkey) . "' AND `Client` = '" . mysql_real_escape_string($clientkey) . "' LIMIT 1;");
  }else{
    echo '<p>Client is not in your case load</p>';
  }
}
...
function AddClientToCL($clientkey, $workerkey, $searchname = null)
{
  if ($searchname)
  {
    ...
    if ($clcheck == false)
    {
      ...
      {
        $client = mysql_fetch_row($clientresult);
        mysql_query("INSERT INTO `caseloads` (`Worker`, `Client`, `Client Name`) VALUES ('".mysql_real_escape_string($workerkey)."', '".mysql_real_escape_string($clientkey)."', '".mysql_real_escape_string($client[0]).' '.mysql_real_escape_string($client[1])."');");
