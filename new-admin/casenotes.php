		...
		<div id="content" <?php if (!isset($_SESSION['clientName'])) echo 'style="margin-right: 0; border-right: none;"'; ?> > 
			<?php 
				if (isset($_SESSION['clientName'])) {
					...
					?>
					<table width=100% cellspacing=0 style="border-spacing: 3px 0; font-size:14px;">
					<tr><th>Date</th><th>Person Contacted/Type of Contact</th><th width=50%>Subject</th></tr>
					<?php
						$casenotes = mysql_query("SELECT * FROM `casenotes` WHERE `Client` =".$_SESSION['client']." ORDER BY `Date` DESC, `Key` DESC LIMIT 0, 30");
						$i = 0;
						if ($casenotes)
						{
							while ($row = mysql_fetch_array($casenotes, MYSQL_ASSOC)) 
							{
								...
								echo '<tr class="d'.($i & 1).'"><td><a href="casenote.php?note='.$row['Key'].'" onClick="window.open(this,\'Case_Notes\',\'width=385,height=425,scrollbars=no,location=no\'); return false;">'.$row['Date'].'</a></td><td>'.$row['Contactor'].' '.$type.' '.$row['Contactee'].'</td><td>'.$row['Subject'].'</td>';
							}
